...
function Search({ reproducirLista, pausar }) {
  ...
  const manejarClickAlbum = (album) => {
    ...
    conPublicidad(() => {
      navigate(
        `/album/${encodeURIComponent(album.name)}/${encodeURIComponent(nombreArtista)}`,
      );
    });
  };
    ...
    const buscar = async () => {
      if (busqueda.trim().length > 2) {
        ...
        try {
          ...
          if (dataAlbumes.results?.albummatches) {
            ...
            let resultados = listaRaw.filter((album) => {
              const nombreLimpio = album.name
                .toLowerCase()
                .replace(
                  /remastered|remaster|deluxe|edition|anniversary|version|special/g,
                  "",
                )
                .trim();
          ...
          const resArtistas = await fetch(
            `/deezer/search/artist?q=${encodeURIComponent(busqueda)}`,
          );
          ...
          const resCanciones = await fetch(
            `/deezer/search?q=${encodeURIComponent(busqueda)}`,
          );
  ...
  return (
    <div className="content">
      <main className="main-header">
        <div className="maina">
          <h1 style={{ color: role === "premium" || role === "admin" ? ' #d0b412' : 'none' }}>Bienvenida/o. ¿Qué vas a escuchar hoy?</h1>
        </div>
      </main>
        ...
        <>
          <div className="search-tabs">
            ...
            >
              Álbumes{" "}
              {albumes.length > 0 && (
                <span className="tab-count">{albumes.length}</span>
              )}
            </button>
            <button
              className={`search-tab ${tab === "artistas" ? "active" : ""}`}
              onClick={() => setTab("artistas")}
            >
              Artistas{" "}
              {artistas.length > 0 && (
                <span className="tab-count">{artistas.length}</span>
              )}
            </button>
            <button
              className={`search-tab ${tab === "canciones" ? "active" : ""}`}
              onClick={() => setTab("canciones")}
            >
              Canciones{" "}
              {canciones.length > 0 && (
                <span className="tab-count">{canciones.length}</span>
              )}
            </button>
          </div>
                    ...
                    >
                      ...
                      <p
                        onClick={(e) => {
                          e.stopPropagation();
                          navigate(
                            `/artist/${encodeURIComponent(ExtractedArtist)}`,
                          );
                        }}
              ...
              {artistas.length === 0 ? (
                ...
                artistas.map((artista, index) => (
                  <div
                    ...
                    onClick={() =>
                      navigate(`/artist/${encodeURIComponent(artista.name)}`)
                    }
                  >
                    <img
                      src={
                    ...
                    <div>
                      ...
                      <div style={{ color: "#aaa", fontSize: "12px" }}>
                        <span
                          onClick={() =>
                            navigate(
                              `/artist/${encodeURIComponent(cancion.artist.name)}`,
                            )
                          }
                          style={{ cursor: "pointer" }}
                          onMouseEnter={(e) => (e.target.style.color = "#fff")}
                    ...
                    >
                      ...
                      <button
                        onClick={() => {
                          if (!esPremium) {
                            alert(
                              "🔒 Necesitás ser usuario Premium para agregar canciones.",
                            );
                            return;
                          }
