...
function Home() {
  ...
  useEffect(() => {
    const fetchHomeData = async () => {
      try {
        // Hacemos todas las peticiones a la vez
        const [resTop, resMetal, resRnb, resPop] = await Promise.all([
          fetch(`https://ws.audioscrobbler.com/2.0/?method=tag.gettopalbums&tag=popular&api_key=${API_KEY}&format=json&limit=12`),
          fetch(`https://ws.audioscrobbler.com/2.0/?method=tag.gettopalbums&tag=heavy metal&api_key=${API_KEY}&format=json&limit=12`),
          fetch(`https://ws.audioscrobbler.com/2.0/?method=tag.gettopalbums&tag=rnb&api_key=${API_KEY}&format=json&limit=12`),
          fetch(`https://ws.audioscrobbler.com/2.0/?method=tag.gettopalbums&tag=pop&api_key=${API_KEY}&format=json&limit=12`)
        ]);
  ...
  // Función para no repetir el mismo código de sección 4 veces
  ...
  return (
    <div className="home-container">
      ...
      {renderSection("Tendencias Globales", topAlbums, topRef)}
